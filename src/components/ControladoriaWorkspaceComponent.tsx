...
export const ControladoriaWorkspaceComponent: React.FC<ControladoriaWorkspaceProps> = ({
  ...
  const [todoistModalSearch, setTodoistModalSearch] = useState("");
  const [todoistModalFilter, setTodoistModalFilter] = useState<"Ativas" | "Concluídas" | "Todas">("Todas");
    ...
    <div className="space-y-6 text-slate-800 animate-fade-in relative">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Pesquise por CNJ, cliente, parte, pasta física, nome ou link da tarefa"
                  value={searchQuery}
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl text-slate-100 overflow-hidden font-sans">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                        {todoistDiagnostic?.route || "/pushes/push-trt-mg/atualizar-controladoria"} • Diagnóstico Técnico
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-950 p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-md space-y-3">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="text-xs font-bold text-white uppercase tracking-wide">Payload Original (Frontend)</h4>
                      </div>
                      <div className="space-y-1.5 text-[11px] font-mono">
                        <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-800/80">
                          <span className="text-slate-500 font-black uppercase text-[9px]">Rota (Trigger)</span>
                          <span className="text-emerald-400 break-all ml-4 text-right">{todoistDiagnostic?.route || "/pushes/push-trt-mg/atualizar-controladoria"}</span>
                        </div>
                        <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-800/80">
                          <span className="text-slate-500 font-black uppercase text-[9px]">Target CNJ</span>
                          <span className="text-amber-300 font-bold">{todoistDiagnostic?.cnj || controladoriaActiveItem?.processNumber || "(vazio)"}</span>
                        </div>
                        <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-800/80">
                          <span className="text-slate-500 font-black uppercase text-[9px]">Target Nome da Tarefa</span>
                          <span className="text-amber-300 font-bold">{todoistDiagnostic?.manualSearchRaw || todoistDiagnostic?.taskName || "(vazio)"}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-md space-y-3">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="text-xs font-bold text-white uppercase tracking-wide">Autenticação & Estado</h4>
                      </div>
                      <div className="space-y-1.5 text-[11px] font-mono">
                        <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-800/80">
                          <span className="text-slate-500 font-black uppercase text-[9px]">Endpoint Chamado</span>
                          <span className="text-indigo-300">/api/todoist/search-all</span>
                        </div>
                        <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-800/80">
                          <span className="text-slate-500 font-black uppercase text-[9px]">Status Interno</span>
                          <span className={todoistDiagnostic?.requestSucceeded ? "text-emerald-400 font-bold" : "text-amber-400 font-bold"}>
                            {todoistDiagnostic?.requestSucceeded ? "SUCESSO" : "FALHA"}
                          </span>
                        </div>
                        <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-800/80">
                          <span className="text-slate-500 font-black uppercase text-[9px]">Estratégia Localizada</span>
                          <span className="text-emerald-400 font-bold">{todoistDiagnostic?.stages?.["6"]?.details?.strategy || "N/A"}</span>
                        </div>
                        <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-800/80">
                          <span className="text-slate-500 font-black uppercase text-[9px]">Tarefas Processadas</span>
                          <span className="text-slate-300">{todoistDiagnostic?.stages?.["6"]?.details?.tasksProcessed || 0}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-2 font-mono text-xs text-slate-300">
                    <div><strong className="text-slate-500 mr-2">Status Final:</strong> <span className={todoistDiagnostic?.status?.includes('SUCCESS') ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'}>{todoistDiagnostic?.status || "N/A"}</span></div>
                    <div><strong className="text-slate-500 mr-2">Trace ID:</strong> <span>{todoistDiagnostic?.traceId || "N/A"}</span></div>
                    {todoistDiagnostic?.failureReason && (
                      <div><strong className="text-red-500 mr-2">Erro Real:</strong> <span className="text-red-400">{todoistDiagnostic.failureReason}</span></div>
                    )}
                    {todoistDiagnostic?.matchFound && (
                      <>
                        <div><strong className="text-emerald-500 mr-2">ID Tarefa Localizada:</strong> <span className="text-emerald-400">{todoistDiagnostic.stages?.["7"]?.details?.chosenTaskId}</span></div>
                        <div><strong className="text-emerald-500 mr-2">Estado Espelho:</strong> <span className="text-emerald-400">{todoistDiagnostic.mirrorReady ? "Renderizado com a tarefa Todoist" : "Aguardando"}</span></div>
                      </>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-sm font-bold text-slate-300 flex items-center gap-2">
                      ...
                      Eventos de Rastreamento (Trace)
                    </h4>
                    {todoistDiagnostic?.events?.length > 0 ? (
                      <div className="space-y-2">
                        {todoistDiagnostic.events.map((evt: any, i: number) => {
                          let badgeColor = "bg-slate-800 text-slate-300 border-slate-700";
                          if (evt.type === "success") badgeColor = "bg-emerald-950/50 text-emerald-400 border-emerald-900/50";
                          if (evt.type === "error") badgeColor = "bg-red-950/50 text-red-400 border-red-900/50";
                          if (evt.type === "warning") badgeColor = "bg-amber-950/50 text-amber-400 border-amber-900/50";
                          if (evt.type === "info") badgeColor = "bg-indigo-950/50 text-indigo-400 border-indigo-900/50";

                          return (
                            <div key={i} className={`text-xs p-3 rounded-xl border font-mono ${badgeColor}`}>
                              <span className="opacity-50 mr-3">[{new Date(evt.timestamp).toLocaleTimeString()}]</span>
                      ...
                      <div className="text-slate-500 text-xs italic">Nenhum evento de rastreamento disponível.</div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 pt-4 border-t border-slate-800">
                        ...
                        {automationLogs.length > 0 && (
                          <>
                            <button
                              ...
                              className="text-[10px] font-semibold text-slate-400 hover:text-indigo-400 bg-slate-900 hover:bg-slate-800 border border-slate-700 px-2.5 py-1 rounded-lg transition"
                            >
                              Copiar logs
                            </button>
                            <button
                              onClick={clearAutomationLogs}
                              className="text-[10px] font-semibold text-slate-400 hover:text-red-400 bg-slate-900 hover:bg-slate-800 border border-slate-700 px-2.5 py-1 rounded-lg transition"
                            >
                              Limpar logs
                            </button>
                          </>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 min-h-[120px]">
                      {automationLogs.length === 0 ? (
                        <div className="text-[11px] text-slate-500 italic py-1 text-center font-mono">
                          Nenhum log de automação registrado ainda.
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 gap-y-2 max-h-64 overflow-y-auto pr-1 custom-scrollbar">
                          {automationLogs.map((log) => {
                            let emoji = "ℹ️";
                            let bgClass = "text-slate-400";
                            if (log.type === "success") {
                              emoji = "✅";
                              bgClass = "text-emerald-400 font-medium";
                            } else if (log.type === "alert") {
                              emoji = "⚠️";
                              bgClass = "text-amber-400 font-medium";
                            } else if (log.type === "error") {
                              emoji = "❌";
                              bgClass = "text-red-400 font-bold";
                            } else if (log.type === "processing") {
                              emoji = "🔄";
                              bgClass = "text-indigo-400 animate-pulse";
                            }
      ...
      {isTodoistSelectionModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-fade-in font-sans">
            {/* Modal Header */}
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-indigo-50/50 to-white">
              <div className="space-y-1">
                <h3 className="text-base font-black text-slate-800 uppercase tracking-wider flex items-center gap-2">
                  ...
                  Seleção Manual de Tarefa
                </h3>
                <p className="text-xs text-slate-500 font-medium">
                  {todoistMultipleTasksFound.length} candidato(s) encontrado(s).
                </p>
              </div>
              <button
            ...
            <div className="p-4 bg-slate-50 border-b border-slate-200 space-y-3">
              <div className="flex gap-2">
                {(["Todas", "Ativas", "Concluídas"] as const).map(f => (
                  <button
                    key={f}
                    onClick={() => setTodoistModalFilter(f)}
                    className={`px-4 py-1.5 rounded-full text-[11px] font-bold tracking-wide transition ${
                      todoistModalFilter === f 
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Filtrar candidatos por título, CNJ, projeto, etiqueta..."
                  value={todoistModalSearch}
                  onChange={e => setTodoistModalSearch(e.target.value)}
                  className="w-full bg-white border border-slate-200 focus:border-indigo-500 rounded-xl pl-9 pr-4 py-2.5 text-xs font-medium outline-none transition"
                />
              </div>
              
              {/* Indicadores do Trace */}
              {todoistDiagnostic && (
                <div className="flex gap-4 text-[10px] font-mono text-slate-500 bg-white border border-slate-200 p-2 rounded-lg">
                  <span><strong className="text-slate-700">Páginas Lidas:</strong> {todoistDiagnostic.events?.filter((e: any) => e.message?.includes('Página') && e.message?.includes('lida')).length || 'N/A'}</span>
                  <span><strong className="text-slate-700">Tarefas Varridas:</strong> {
                    todoistDiagnostic.events?.find((e: any) => e.message?.includes('Varredura concluída'))?.message?.match(/(\d+) tarefas totais/)?.[1] || 'N/A'
                  }</span>
                </div>
              )}
            </div>
            ...
            {/* Modal Options List */}
            <div className="p-6 overflow-y-auto space-y-4 flex-1 bg-slate-50/50 custom-scrollbar">
              {todoistMultipleTasksFound
                .filter(task => {
                   if (todoistModalFilter === "Ativas") return !task.is_completed;
                   if (todoistModalFilter === "Concluídas") return task.is_completed;
                   return true;
                })
                .filter(task => {
                   if (!todoistModalSearch.trim()) return true;
                   const terms = todoistModalSearch.toLowerCase();
                   const t = String(task.content || "").toLowerCase();
                   const d = String(task.description || "").toLowerCase();
                   const p = String(todoistProjects.find(pr => pr.id === task.project_id)?.name || "").toLowerCase();
                   return t.includes(terms) || d.includes(terms) || p.includes(terms);
                })
                .map((task) => {
                  ...
                  >
                    <div className="flex justify-between items-start gap-4">
                      <div className="space-y-1.5 flex-1">
                        <h4 className={`text-sm font-bold leading-snug ${isCompleted ? 'text-slate-400 line-through' : 'text-slate-900'}`}>
                          {task.content}
                        </h4>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[10px] text-slate-500">
                          ...
                          <span className={`px-1.5 py-0.5 rounded font-bold border ${isCompleted ? 'bg-slate-100 text-slate-500 border-slate-200' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>
                      ...
                      <button
                        onClick={async () => {
                          // The instruction says: "1. Buscar a tarefa completa pelo ID. 2. Atualizar todoistLinkedTask."
                          setIsTodoistSelectionModalOpen(false);
                          
                          // Set loading immediately to show progress in Mirror
                          setTodoistLoadingLocal(true);
                          try {
                            const res = await fetch(`/api/todoist/tasks/${task.id}`);
                            if (res.ok) {
                               const fullTask = await res.json();
                               handleSelectTask(fullTask);
                            } else {
                               handleSelectTask(task); // fallback
                            }
                          } catch (e) {
                             handleSelectTask(task); // fallback
                          } finally {
                        ...
                        className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-[10px] uppercase tracking-wider py-2.5 px-4 rounded-xl transition shadow-sm hover:shadow flex items-center gap-1.5 whitespace-nowrap shrink-0"
                      >
                        <Check className="h-3.5 w-3.5 stroke-[3]" />
                        Selecionar esta tarefa
                      </button>
                    </div>
                    
                    {/* Trecho relevante (description) se houver */}
                    {task.description && (
                       <div className="text-[10px] text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100 line-clamp-2">
                         <strong className="text-slate-600 mr-1">Descrição:</strong>
                         {task.description}
                       </div>
                    )}
                    ...
                    {task.labels && task.labels.length > 0 && (
                      <div className="flex flex-wrap gap-1 text-[9px]">
                        {task.labels.map((lbl: string) => (
                          <span key={lbl} className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full border border-slate-200">
                            #{lbl}
                          </span>
                        ))}
                      </div>
              ...
              {todoistMultipleTasksFound.length === 0 && (
