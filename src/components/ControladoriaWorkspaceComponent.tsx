  ...
  return (
    <div className="space-y-6 text-slate-800 animate-fade-in relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* COLUMN 2: CENTER PANEL (PUBLICATION & EMAIL VIEWER) (4 Cols) */}
        <div className="lg:col-span-4 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-5">
          <div className="border-b border-slate-200 pb-3 flex items-center justify-between">
            <h3 className="text-xs font-black uppercase tracking-wider text-slate-900 flex items-center gap-1.5">
              <Inbox className="h-4 w-4 text-indigo-500" /> Email original
            </h3>
            <span className="bg-indigo-50 text-indigo-800 text-[9px] px-2 py-0.5 rounded-full font-black border border-indigo-150 uppercase tracking-wide">
              Conteúdo Integral
            </span>
          </div>

          {/* Email / Publication Header Details */}
          <div className="space-y-2 bg-slate-50 border border-slate-100 rounded-xl p-3 text-xs">
            <div className="flex justify-between">
              <span className="text-slate-400 font-medium">Assunto:</span>
              <span className="text-slate-800 font-bold truncate max-w-[200px]" title={controladoriaActiveItem.subject || controladoriaActiveItem.title}>
                {controladoriaActiveItem.subject || controladoriaActiveItem.title || "E-mail de Push"}
              </span>
            </div>
            {controladoriaActiveItem.from && (
              <div className="flex justify-between">
                <span className="text-slate-400 font-medium">Remetente:</span>
                <span className="text-slate-800 font-medium truncate max-w-[200px]" title={controladoriaActiveItem.from}>
                  {controladoriaActiveItem.from}
                </span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-slate-400 font-medium">Data Recebimento:</span>
          ...
          {/* Scrollable Email Body Text or Rich HTML View */}
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-slate-100 pb-1">
              <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Visualizador de E-mail</label>
              <span className="text-[9px] text-slate-400 italic">
                {viewRawCode ? "Modo Técnico (Raw)" : controladoriaActiveItem.htmlBody ? "Formato HTML Original" : "Modo Texto"}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              <a
                href={`https://mail.google.com/mail/u/0/#inbox/${controladoriaActiveItem.id}`}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-[9px] uppercase tracking-wider py-1 px-2.5 rounded-lg transition flex items-center gap-1 border border-indigo-200"
              >
              ...
              <button
                onClick={() => {
                  navigator.clipboard.writeText(controladoriaActiveItem.bodyText || controladoriaActiveItem.snippet || "");
                  addSystemLog("success", "Texto limpo copiado com sucesso!");
                }}
                className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold text-[9px] uppercase tracking-wider py-1 px-2.5 rounded-lg transition flex items-center gap-1 border border-emerald-200"
              >
              ...
              <button
                onClick={() => setViewRawCode(!viewRawCode)}
                className={`font-bold text-[9px] uppercase tracking-wider py-1 px-2.5 rounded-lg transition flex items-center gap-1 border ${
                  viewRawCode 
              ...
              >
                ...
                {viewRawCode ? "Ver HTML" : "Ver código original"}
              </button>
            </div>

            {/* Content Container */}
            {viewRawCode ? (
              <div className="w-full bg-slate-900 border border-slate-950 rounded-xl p-3.5 h-[340px] overflow-y-auto shadow-inner text-[10px] text-emerald-400 font-mono leading-relaxed select-text whitespace-pre-wrap">
                {controladoriaActiveItem.htmlBody || controladoriaActiveItem.bodyText || "Nenhum código raw disponível."}
              </div>
            ) : controladoriaActiveItem.htmlBody ? (
              <iframe
                srcDoc={`
                  <!DOCTYPE html>
                  <html>
                ...
                `}
                title="Visualização do E-mail Original"
                className="w-full h-[340px] bg-white border border-slate-200 rounded-xl shadow-inner"
                sandbox="allow-popups allow-popups-to-escape-sandbox"
              />
            ) : (
              <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 h-[340px] overflow-y-auto shadow-inner text-xs text-slate-700 leading-relaxed font-sans whitespace-pre-wrap select-text">
                {controladoriaActiveItem.bodyText || controladoriaActiveItem.snippet || (
                  <span className="text-slate-400 italic">Sem conteúdo textual disponível para exibição direta.</span>
                )}
              </div>
            )}
          </div>
