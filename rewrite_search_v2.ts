...
const searchAllEndStr = 'app.get("/api/todoist/tasks", async (req: any, res) => {';
const searchAllEndMatch = serverTs.indexOf(searchAllEndStr, searchAllStart);
if (searchAllEndMatch === -1) throw new Error("Could not find end");
...
serverTs = serverTs.substring(0, searchAllStart) + newSearchAll + serverTs.substring(searchAllEndMatch);
